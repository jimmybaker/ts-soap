...
var NamespaceContext_1 = require("./NamespaceContext");
var WSDL = /** @class */ (function () {
    ...
    WSDL.prototype.xmlToObject = function (xml, callback) {
        ...
        p.onopentag = function (node) {
            ...
            var name = index_1.splitQName(nsName).name;
            var attributeName;
            ...
            for (attributeName in attrs) {
                if (/^xmlns:|^xmlns$/.test(attributeName)) {
                    xmlns[index_1.splitQName(attributeName).name] = attrs[attributeName];
                    continue;
                }
            ...
            for (attributeName in elementAttributes) {
                var res = index_1.splitQName(attributeName);
                if (res.name === "nil" &&
            ...
            if (xsiType) {
                var type = index_1.splitQName(xsiType);
                var typeURI;
                if (type.prefix === index_1.TNS_PREFIX) {
        ...
        p.onclosetag = function (nsName) {
            var cur = stack.pop(), obj = cur.object, top = stack[stack.length - 1], topObject = top.object, topSchema = top.schema, name = index_1.splitQName(nsName).name;
        ...
        p.ontext = function (text) {
            ...
            var top = stack[stack.length - 1];
            var name = index_1.splitQName(top.schema).name, value;
            if (self.options &&
    ...
    WSDL.prototype.objectToRpcXML = function (name, params, nsPrefix, nsURI, isParts) {
        ...
        nsPrefix = nsPrefix === index_1.TNS_PREFIX ? "" : nsPrefix + ":";
    ...
    WSDL.prototype.objectToXML = function (obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
        ...
        var parts = [];
        var prefixNamespace = (nsPrefix || qualified) && nsPrefix !== index_1.TNS_PREFIX;
        var xmlnsAttrib = "";
        if (nsURI && isFirst) {
            if (self.options.overrideRootElement &&
        ...
        if (!nsContext) {
            nsContext = new NamespaceContext_1.default();
        ...
        if (Array.isArray(obj)) {
            ...
            for (name in obj) {
                ...
                if (name === self.options.valueKey) {
                    nsContext.popContext();
                    return index_1.xmlEscape(obj[name]);
                }
                var child = obj[name];
                        ...
                        if (schema) {
                            ...
                            if (childSchemaObject &&
                               ...
                               */
                                ...
                                if (elementQName) {
                                    elementQName = index_1.splitQName(elementQName);
                                    childName = elementQName.name;
                                    if (elementQName.prefix === index_1.TNS_PREFIX) {
                                        // Local element
                                ...
                                if (childSchemaObject.$type) {
                                    var typeQName = index_1.splitQName(childSchemaObject.$type);
                            ...
                            else if (obj[self.options.attributesKey] &&
            ...
            parts.push(self.options.escapeXML ? index_1.xmlEscape(obj) : obj);
        }
        nsContext.popContext();
        return parts.join("");
    };
        ...
        if (attrObj) {
            for (var attrKey in attrObj) {
                ...
                if (attrKey === "xsi_type") {
                    ...
                    attr += " " + attrKey + '="' + index_1.xmlEscape(attrObj[attrKey]) + '"';
                }
            }
        }
        return attr;
    ...
    WSDL.prototype.findChildSchemaObject = function (parameterTypeObj, childName, backtrace) {
        ...
        if (object.$ref) {
            ref = index_1.splitQName(object.$ref);
        ...
        if (object.$type && backtrace.length === 1) {
            var typeInfo = index_1.splitQName(object.$type);
            if (typeInfo.prefix === index_1.TNS_PREFIX) {
        ...
        if (object.children) {
            for (i = 0, child; (child = object.children[i]); i++) {
                ...
                if (child.$base) {
                    var baseQName = index_1.splitQName(child.$base);
                    var childNameSpace = baseQName.prefix === index_1.TNS_PREFIX ? "" : baseQName.prefix;
                    childNsURI =
            ...
            if (top) {
                try {
                    ...
                    if (self.options.strict) {
                        ...
                        stack.push(new Element_1.default(nsName, attrs, options, schemaAttrs));
                    }
                }
            }
            else {
                if (name === "definitions") {
                    root = new DefinitionsElement_1.default(nsName, attrs, options);
                    stack.push(root);
                }
                else if (name === "schema") {
                    // Shim a structure in here to allow the proper objects to be created when merging back.
                    root = new DefinitionsElement_1.default("definitions", {}, {});
                    types = new TypesElement_1.default("types", {}, {});
                    schema = new SchemaElement_1.default(nsName, attrs, options);
    ...
    WSDL.prototype._xmlnsMap = function () {
        ...
        for (var alias in xmlns) {
            if (alias === "" || alias === index_1.TNS_PREFIX) {
                continue;
            }
            var ns = xmlns[alias];
            switch (ns) {
